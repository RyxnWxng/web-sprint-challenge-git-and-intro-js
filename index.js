...
function addArtist(artists, name, years, genere, nationality, bio) {
  const newArtists = [...artists];
  const newArray = {
    name: name, 
    years: years,
...
function artistByCountry(artists, nation){
  const artistsCountry = []
  for (let i = 0; i < artists.length; i++){
    if (artists[i].nationality === nation)
    artistsCountry.push(artists[i].name)
  }
